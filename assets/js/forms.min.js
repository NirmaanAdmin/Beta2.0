$(function () {
  ...
  $(".save_changes_settings_single_form").on("click", function (e) {
    ...
    var selectRequired = ['project_id', 'assigned', 'priority'];
  ...
  $("body").on('change', '#project_id', function () {
    ...
    var contact_db_id = $('input[name="contact_db_id"]').val();
    if (project_id) {
      $('#contactid').empty().selectpicker('refresh');
      $.post(admin_url + 'forms/find_project_contact', { 'project_id': project_id }).done(function (response) {
        response = JSON.parse(response);
        if (response.length > 0) {
          $.each(response, function (idx, contact) {
            var contactid = $('#contactid');
            if (contact_db_id == contact.id) {
              contactid.prepend('<option value="' + contact.id + '" selected>' + contact.full_name + '</option>');
            } else {
              contactid.prepend('<option value="' + contact.id + '">' + contact.full_name + '</option>');
            }
          });
      ...
      $.post(admin_url + 'forms/form_change_data/', {
        ...
        if (response.contact_data) {
          ...
          if (response.contact_data.ticket_emails == '0') {
            show_form_no_contact_email_warning(response.contact_data.userid, response.contact_data.id);
          } else {
