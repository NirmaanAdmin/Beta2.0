...
function assign_unawarded_capex(id) {
  $.post(admin_url + "estimates/assign_unawarded_capex", {
    ...
    if (response.success) {
      ...
      $('.unawarded-capex-body').html(response.html);
...
function calculate_unawarded_capex() {
  var total_budgeted_cost = 0,
  total_awarded_capex = 0,
  total_unawarded_capex = 0,
  total_unallocated_cost = 0;
  var rows = $(".unawarded-capex-body .row");
  $.each(rows, function () {
    var row = $(this);
    var budgeted_cost = parseFloat(row.find(".all_budgeted_cost input").val()) || 0;
    var awarded_capex = parseFloat(row.find(".all_awarded_capex input").val()) || 0;
    var unawarded_capex = parseFloat(row.find(".all_unawarded_capex input").val()) || 0;
    var unallocated_cost = (awarded_capex + unawarded_capex) - budgeted_cost;
    row.find(".all_unallocated_cost input").val(unallocated_cost.toFixed(2));
    total_budgeted_cost += budgeted_cost;
    total_awarded_capex += awarded_capex;
    total_unawarded_capex += unawarded_capex;
    total_unallocated_cost += unallocated_cost;
  });
  $(".total_budgeted_cost").html(format_money(total_budgeted_cost));
  $(".total_awarded_capex").html(format_money(total_awarded_capex));
  $(".total_unawarded_capex").html(format_money(total_unawarded_capex));
  $(".total_unallocated_cost").html(
    format_money(total_unallocated_cost) +
    hidden_input(
      "total_unalloc_cost",
