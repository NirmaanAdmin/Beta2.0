...
$(function () {
  ...
  $("form")
    .not(
      "#single-ticket-form,#single-form-form,#calendar-event-form,#proposal-form, #department-form"
    )
    .areYouSure();
  ...
  $("body").on("submit", "._transaction_form", function () {
    ...
    if (
      ...
      if ($itemsTable.length && $itemsTable.find(".item").length === 0) {
        ...
        alert_float('warning', 'Please enter the item details under at least one budget head.');
        return false;
      }
    }
  ...
  $("body").on("click", ".transaction-submit", function () {
    ...
    var missing_pur_items = [];
    var missing_area_items = [];
    $('.pur_item_name select.item-select').each(function () {
      var itemVal = $(this).val().trim();
      var annexureName = $(this).closest('tr').closest('.s_table').prevAll('.col-md-4').find('.annexure_name:first').text().trim();
      var annexure_overall_area = $(this).closest('tr').closest('.s_table').prevAll('.col-md-4').find('#overall_budget_area_id').val().trim();
      if (itemVal === '') {
        missing_pur_items.push(annexureName);
      }
      if (annexure_overall_area === '') {
        missing_area_items.push(annexureName);
      }
    });
    var unique_pur_items = [...new Set(missing_pur_items)];
    var unique_area_items = [...new Set(missing_area_items)];
    var lock_budget = $('input[name="lock_budget"]').data('lock-budget');
    var last_revision = $('input[name="last_revision"]').data('last-revision');
    var next_revision = $('input[name="next_revision"]').data('next-revision');
    if (unique_pur_items.length > 0) {
      alert_float('warning', 'Please enter item details in ' + unique_pur_items.join(', '));
    } else if (unique_area_items.length > 0) {
      alert_float('warning', 'Please enter overall area in ' + unique_area_items.join(', '));
    } else if (lock_budget == 1 && last_revision !== '' && next_revision !== '') {
      Swal.fire({
        title: 'Please Select Revision',
        text: 'Do you want to consider as Revision ' + last_revision + ' or Revision ' + next_revision + '?',
        icon: 'question',
        showCancelButton: true,
        showDenyButton: true,
        confirmButtonText: 'Revision ' + last_revision,
        denyButtonText: 'Revision ' + next_revision,
        cancelButtonText: 'Cancel',
        customClass: {
          confirmButton: 'swal-button-style',
          denyButton: 'swal-button-style'
        }
      }).then((result) => {
        if (result.isConfirmed) {
          form.submit();
        } else if (result.isDenied) {
          form.append(hidden_input("next_revision", "true"));
          form.submit();
        }
      });
    } else {
  ...
  $("body").on("change", ".invoice #project_id", function () {
    ...
    if (project_id !== "") {
      requestGetJSON("invoices/get_payment_modes_by_project/" + project_id).done(
        function (response) {
          if (response.data) {
            $('select[name="allowed_payment_modes[]"]').val([]).trigger('change');
            $('select[name="allowed_payment_modes[]"]').val(response.data.toString().split(',')).trigger('change');
          } else {
            $('select[name="allowed_payment_modes[]"]').val([]).trigger('change');
          }
        }
...
function initDataTable(
  ...
  if ($("table.table-invoices").length > 0 || $("table.table-table_pur_invoices").length > 0 || $("table.table-table_unawarded_capex_items").length > 0 || lastSegment === 'table_critical_tracker') {
  ...
  var dtSettings = {
    ...
    fnCreatedRow: function (nRow, aData, iDataIndex) {
      // If tooltips found
      if (rowgroup) {
        var updatedData = aData.filter(element => element.trim() !== '');
        if (updatedData.length == 1) {
          $('td', nRow).eq(0).attr('colspan', 9).html(`<b>${updatedData[0]}</b>`);
          $('td', nRow).not(':first').remove();
        }
      } else {
  ...
  mywindow.document.write(`
      <style>
          @media print {
              ...
              #tab_expense { width: 100%; border: 1px solid #ddd; padding: 10px; }
...
function print_form_message(id, type) {
  var printMessage = $("[data-" + type + '-id="' + id + '"]').html();
  var printSubject = $("#form_subject").text().trim();
  var mywindow = _create_print_window(printSubject);
  mywindow.document.write("<html><head><title>" + app.lang.form + "</title>");
  _add_print_window_default_styles(mywindow);
  mywindow.document.write("</head><body>");
  mywindow.document.write("<h1>" + printSubject + "</h1>");
  mywindow.document.write(printMessage);
...
// Table forms
function init_table_forms(manual) {
  // Single form is for other forms from user
  if (
    typeof manual == "undefined" &&
    ($("body").hasClass("dashboard") || $("body").hasClass("single-form"))
  ) {
    return false;
  }
  if ($("body").find(".forms-table").length === 0) {
    return;
  }
  var FormServerParams = {},
    Forms_Filters = $("._hidden_inputs._filters.forms_filters input");
  var forms_date_created_index = $(
    "table.forms-table thead .form_created_column"
  ).index();
  $.each(Forms_Filters, function () {
    FormServerParams[$(this).attr("name")] =
      '[name="' + $(this).attr("name") + '"]';
  });
  FormServerParams["project_id"] = '[name="project_id"]';
  var formsTableNotSortable = [0]; // bulk actions
  var _forms_table_url = admin_url + "forms";
  if ($("body").hasClass("forms-page")) {
    _forms_table_url += "?bulk_actions=true";
  }
  _table_api = initDataTable(
    ".forms-table",
    _forms_table_url,
    formsTableNotSortable,
    formsTableNotSortable,
    FormServerParams,
    [forms_date_created_index, "desc"]
  );
  if (_table_api && $("body").hasClass("dashboard")) {
    var notVisibleDashboardDefault = [4, forms_date_created_index, 5, 6];
...
function calculate_total() {
  ...
  if ($("#tab_final_invoice").length > 0) {
    return false;
  }
  if ($("#tab_final_estimate").length > 0) {
    return false;
  }
  ...
  $(row)
    ...
    .addClass("animated fadeOut", function () {
      setTimeout(function () {
        ...
        calculate_total();
        var annexure = $('.annexture-list li.active a').attr('aria-controls');
        if (annexure) {
          calculate_invoice_total(annexure);
        }
      }, 100);
    });
...
function format_money(total, excludeSymbol = false, currency = 'INR') {
  // Validate
  ...
  // Symbol placement
  return config.placement === 'after' ? formattedAmount + '' + config.symbol : config.symbol + '' + formattedAmount;
}

function amount_format(num, type = 1, symbol = '') {
  const isNegative = num < 0;
  num = isNegative ? Math.abs(num) : num;
  const num_full = Number(num).toFixed(2);
  let num_str = num_full.toString();
  if (num_str.includes('.')) {
    num_str = num_str.substring(0, num_str.indexOf('.'));
  }

  if (type === 1) {
    let explrestunits = '';
    if (num_str.length > 3) {
      const lastthree = num_str.slice(-3);
      const restunits = num_str.slice(0, -3);
      let formattedRestunits = restunits.length % 2 === 1 ? '0' + restunits : restunits;
      const expunit = formattedRestunits.match(/.{1,2}/g);
      ...
      let sign = isNegative ? "-" : "";
      return symbol + sign + explrestunits + lastthree + num_full.slice(-3);
    } else {
      let sign = isNegative ? "-" : "";
      return symbol + sign + num_str + num_full.slice(-3);
    }
  }
}
...
$(document).on('click', '.annexture-list a', function (e) {
  e.preventDefault();
  var annexureKey = $(this).attr('aria-controls');
  calculate_invoice_total(annexureKey);
});

function calculate_invoice_total(annexure) {
  var calculated_tax,
    taxrate,
    item_taxes,
    row,
    ...
    quantity = 1,
    total_discount_calculated = 0,
    rows = $("#" + annexure + " .table.has-calculations tbody tr.item"),
    discount_area = $("#discount_area"),
    annexture_subtotal = $("#" + annexure + " tr#subtotal"),
    adjustment = parseFloat($('input[name="adjustment"]').val()) || 0,
    discount_percent = parseFloat($('input[name="discount_percent"]').val()) || 0,
    discount_fixed = parseFloat($('input[name="discount_total"]').val()) || 0,
    discount_total_type = $(".discount-total-type.selected"),
    discount_type = $('select[name="discount_type"]').val();
  var selected_currency = $('select[name="currency"] option:selected').text() || 'INR';

  $.each(rows, function () {
    row = $(this);
    quantity = parseFloat(row.find("[data-quantity]").val()) || 1;

    _amount = parseFloat(row.find("td.rate input").val()) * quantity;
    _amount = isNaN(_amount) ? 0 : _amount; // Ensure valid number
    subtotal += _amount;
    item_taxes = parseFloat(row.find("td.taxrate input").val()) || 0;
    row.find("td.amount").html(format_money(_amount + item_taxes, false, selected_currency));
    total_tax += item_taxes;
  });

  total = subtotal + total_tax;

  $(".annexture_subtotal").html(format_money(subtotal, false, selected_currency));
  $(".annexture_tax").html(format_money(total_tax, false, selected_currency));
  $(".annexture_total").html(format_money(total, false, selected_currency));

  // Trigger event after total calculation
  $(document).trigger("sales-total-calculated");
}


$("body").on("change", 'input[name="invoice_bill_expenses[]"]', function () {
  ...
  if (invoiceid != '' && expenseid != '') {
    if (confirm("Are you sure?")) {
      $.post(admin_url + 'expenses/applied_to_invoice', { invoice_id: invoiceid, expense_id: expenseid }).done(function (response) {
        response = JSON.parse(response);
        if (response.status) {
          alert_float('success', response.message);
        } else {
...
$(document).on('click', '.estimate-annexture-list a', function (e) {
  e.preventDefault();
  var annexureKey = $(this).attr('aria-controls');
  calculate_estimate_total(annexureKey);
});

function calculate_estimate_total(annexure = '') {
  if (empty(annexure)) {
    annexure = $('.detailed-costing-tab.active').attr('id');
  }
  var calculated_tax,
    taxrate,
    item_taxes,
    ...
    rows = $('#' + annexure + ' .table.has-calculations tbody tr.item'),
    discount_area = $("#discount_area"),
    annexure_subtotal = $('#' + annexure + ' tr#subtotal'),
    adjustment = $('input[name="adjustment"]').val(),
    discount_percent = $('input[name="discount_percent"]').val(),
    discount_fixed = $('input[name="discount_total"]').val(),
    discount_total_type = $(".discount-total-type.selected"),
    discount_type = $('select[name="discount_type"]').val(),
    overall_budget_area = $('#' + annexure + ' #overall_budget_area_id').val();
  overall_budget_area = overall_budget_area || 1;
  ...
  total = total + subtotal;
  subtotal = subtotal / overall_budget_area;
  ...
  $(".discount-total").html(discount_html);
  $(".adjustment").html(format_money(adjustment));
  $(".annexure_subtotal").html(format_money(subtotal));
  $(".annexure_total").html(format_money(total));

  $(document).trigger("sales-total-calculated");
}

// Deletes estimate items
function delete_estimate_item(row, itemid) {
  var annexure = $('.detailed-costing-tab.active').attr('id');
  $(row)
    .parents("tr")
    .addClass("animated fadeOut", function () {
      setTimeout(function () {
        $(row).parents("tr").remove();
        calculate_estimate_total(annexure);
      }, 50);
    });
...
// Add estimate item to preview
function add_estimate_item_to_preview(id, tab) {
  requestGetJSON("invoice_items/get_item_by_id/" + id).done(function (
    response
  ) {
    clear_estimate_item_preview_values(tab);

    $('#' + tab + ' textarea[name="description"]').val(response.description);
    $('#' + tab + ' textarea[name="long_description"]').val(
      response.long_description.replace(/(<|&lt;)br\s*\/*(>|&gt;)/g, " ")
    );

    $('#' + tab + ' input[name="quantity"]').val(1);
    ...
    $('#' + tab + ' select[name="taxname"]').selectpicker("val", taxSelectedArray);
    $('#' + tab + ' input[name="unit"]').val(response.unit);
    ...
    var baseCurency = $currency.attr("data-base");
    var selectedCurrency = $currency.find("option:selected").val();
    var $rateInputPreview = $('#' + tab + ' input[name="rate"]');
...
$("body").on("change", 'select.tax', function () {
  var annexure = $('.tab-pane.active').attr('id');
  calculate_estimate_total(annexure);
});
...
// Append the added items to the preview to the table as items
function add_estimate_item_to_table(data, itemid, merge_invoice, bill_expense) {
  // If not custom data passed get from the preview
  var tab = $('.detailed-costing-tab.active').attr('id');
  var annexure_id = $('.detailed-costing-tab.active').attr('data-id');

  data =
    typeof data == "undefined" || data == "undefined"
      ? get_estimate_item_preview_values(tab)
      : data;
  ...
  var unit_id = "newitems[" + item_key + "][unit_id]";
  var item_name = "newitems[" + item_key + "][item_name]";
  var item_code = data.item_code ? data.item_code : 9799;
  var sub_head = "newitems[" + item_key + "][sub_head]";
  var area = "newitems[" + item_key + "][area][]";
  $("body").append('<div class="dt-loader"></div>');
  var regex = /<br[^>]*>/gi;
  $.when(
    get_estimate_unit_dropdown(unit_id, data.unit_id),
    get_estimate_purchase_items(item_code, item_name),
    get_sub_head_dropdown(sub_head, data.sub_head),
    get_area_dropdown(area, data.area)
  ).done(function (estimate_unit_dropdown, estimate_purchase_dropdown, sub_head_dropdown, area_dropdown) {
    // order input
    table_row +=
      '<input type="hidden" class="order" name="newitems[' +
      item_key +
      '][order]" value=' + item_key + '>';

    table_row +=
      '<input type="hidden" class="annexure" name="newitems[' +
      item_key +
      '][annexure]" value=' + annexure_id + '>';

    table_row += "</td>";

    if (item_code) {
      table_row += '<td class="pur_item_name">' + estimate_purchase_dropdown[0] + '</td>';
    } else {
      table_row += '<td class="pur_item_name"><select id="' + item_name + '" name="' + item_name + '" data-selected-id="" class="form-control selectpicker item-select" data-live-search="true" ><option value="">Type at least 3 letters...</option></select></td>';
    }
    ...
    table_row += '<td class="sub_head">' + sub_head_dropdown[0] + '</td>';

    table_row += '<td class="area">' + area_dropdown[0] + '</td>';

    table_row +=
      '<td><input type="number" min="0" onblur="calculate_estimate_total();" onchange="calculate_estimate_total();" data-quantity name="newitems[' +
      item_key +
      '][qty]" value="' +
      data.qty +
      '" class="form-control">' + estimate_unit_dropdown[0] + '</td>';
    ...
    table_row +=
      '<td class="rate"><input type="number" data-toggle="tooltip" title="' +
      app.lang.item_field_not_formatted +
      '" onblur="calculate_estimate_total();" onchange="calculate_estimate_total();" name="newitems[' +
      item_key +
      '][rate]" value="' +
      data.rate +
      '" class="form-control"></td>';
    ...
    table_row +=
      '<td><textarea name="newitems[' +
      item_key +
      '][remarks]" class="form-control item_remarks" rows="5">' +
      data.remarks.replace(regex, "\n") +
      "</textarea></td>";

    table_row +=
      '<td><a href="#" class="btn btn-danger pull-left" onclick="delete_estimate_item(this,' +
      itemid +
      '); return false;"><i class="fa fa-trash"></i></a></td>';

    table_row += "</tr>";

    $('#' + tab + ' table.items tbody').append(table_row);

    var lastRow = $('#' + tab + ' table.items tbody tr:last');
    lastRow.find('.sub_head select').prop('selectedIndex', 1);
    lastRow.find('.area select').prop('selectedIndex', 1);
    lastRow.find('.sub_head select').selectpicker('refresh');
    lastRow.find('.area select').selectpicker('refresh');
...
function get_estimate_purchase_items(name, value) {
  return $.post(admin_url + "estimates/get_estimate_purchase_items", {
    name: name,
    value: value,
  });
}

function get_sub_head_dropdown(name, value) {
  return $.post(admin_url + "costplanning/get_sub_head_dropdown", {
    name: name,
    value: value,
  });
}

function get_area_dropdown(name, value) {
  return $.post(admin_url + "estimates/get_area_dropdown", {
    name: name,
    value: value,
  });
}
...
// Get the preview main values
function get_estimate_item_preview_values(tab) {
  var response = {};
  response.item_name = $('#' + tab + ' select[name="item_name"]').val();
  response.item_code = $('#' + tab + ' input[name="item_code"]').val();
  response.description = $('#' + tab + ' textarea[name="description"]').val();
  response.long_description = $('#' + tab + ' textarea[name="long_description"]').val();
  response.sub_head = $('#' + tab + ' select[name="sub_head"]').val();
  response.area = $('#' + tab + ' select[name="area[]"]').val();
  response.qty = $('#' + tab + ' input[name="quantity"]').val();
  response.rate = $('#' + tab + ' input[name="rate"]').val();
  response.unit = $('#' + tab + ' input[name="unit"]').val();
  response.unit_name = $('#' + tab + ' select[name="unit_name"]').selectpicker("val");
  response.remarks = $('#' + tab + ' textarea[name="remarks"]').val();
  response.unit_id = $('#' + tab + ' select[name="unit_id"]').selectpicker("val");
  return response;
}

function clear_estimate_item_preview_values(tab) {
  var previewArea = $('#' + tab + '').find("tr").eq(1);
  previewArea.find("textarea").val(""); // includes cf
  previewArea.find('select[name="sub_head"]').selectpicker("val", "");
  previewArea.find('select[name="area[]"]').selectpicker("val", "");
  previewArea.find('input[name="quantity"]').val(1);
  previewArea.find('input[name="rate"]').val("");
  previewArea.find('input[name="unit"]').val("");
  previewArea.find('select[name="unit_name"]').selectpicker("val", "");
  previewArea.find('select[name="unit_id"]').selectpicker("val", "");
  previewArea.find('select[name="item_name"]').selectpicker("val", "");
  previewArea.find('input[name="item_code"]').val("");
}

function get_area_working_item_preview_values(tab) {
  var response = {};
  response.area_description = $('#' + tab + ' textarea[name="area_description"]').val();
  response.area_length = $('#' + tab + ' input[name="area_length"]').val();
  response.area_width = $('#' + tab + ' input[name="area_width"]').val();
  response.carpet_area = $('#' + tab + ' input[name="carpet_area"]').val();
  return response;
}

function clear_area_working_item_preview_values(tab) {
  var previewArea = $('#' + tab + '').find("tr").eq(1);
  previewArea.find("textarea").val("");
  previewArea.find('input').val("");
}

var tabAreaKeyCounts = 0;
function getAreaNextItemKey() {
  if (!tabAreaKeyCounts) {
    tabAreaKeyCounts = 0;
  }
  tabAreaKeyCounts += 1;
  return tabAreaKeyCounts;
}

function add_area_working_item_to_table(data, itemid, merge_invoice, bill_expense) {

  var tab = $('.area_working_tab.active').attr('id');
  var area_id = $('.area_working_tab.active').attr('data-id');
  data =
    typeof data == "undefined" || data == "undefined"
      ? get_area_working_item_preview_values(tab)
      : data;

  var table_row = "";
  var item_key = getAreaNextItemKey();

  var carpet_area = data.area_length * data.area_width;

  var selectedLabel = $('input[name="show_aw_unit"]:checked').next('label').text();
  var selectedLabelupdated = selectedLabel.replace('2', '');

  table_row += '<tr class="item">';

  table_row +=
    '<input type="hidden" name="newareaworkingitems[' +
    item_key +
    '][area_id]" value=' + area_id + '>';

  table_row += '<td><textarea name="newareaworkingitems[' +
    item_key +
    '][area_description]" class="form-control" rows="4">' +
    data.area_description +
    "</textarea></td>";

  table_row +=
    '<td><input type="number" onblur="calculate_area_working_total();" onchange="calculate_area_working_total();" name="newareaworkingitems[' +
    item_key +
    '][area_length]" value="' +
    data.area_length +
    '" class="form-control" id="area_length"><br><span class="show_aw_unit_se">' + selectedLabelupdated + '</span></td>';
  ...
  table_row +=
    ...
    '][area_width]" value="' +
    data.area_width +
    '" class="form-control" id="area_width"><br><span class="show_aw_unit_se">' + selectedLabelupdated + '</span></td>';

  table_row += '<td>' + carpet_area + '</td>';

  table_row +=
    '<td><a href="#" class="btn btn-danger pull-left" onclick="delete_area_working_item(this,' +
    itemid +
    '); return false;"><i class="fa fa-trash"></i></a></td>';
  ...
  setTimeout(function () {
    calculate_area_working_total();
  }, 15);

  init_selectpicker();
  init_datepicker();
  init_color_pickers();
  clear_area_working_item_preview_values(tab);
  return true;
}

function get_estimate_unit_dropdown(name, value) {
  return $.post(admin_url + "costplanning/get_area_unit_dropdown", {
    name: name,
    value: value,
  });
}

function calculate_area_working_total() {
  var tab = $('.area_working_tab.active').attr('id');
  var total_length = 0;
  var total_width = 0;
  var total_carpet_area = 0;
  var rows = $('#' + tab + ' tbody.area_working tr.item');
  $.each(rows, function () {
    var area_length = $(this).find("#area_length").val();
    var area_width = $(this).find("#area_width").val();
    var carpet_area = 0;
    area_length = parseFloat(area_length) || 0;
    area_width = parseFloat(area_width) || 0;
    carpet_area = area_length * area_width;
    total_length += area_length;
    total_width += area_width;
    total_carpet_area += carpet_area;
    $(this).find(".carpet_area").html(carpet_area);
  });

  if (Number.isInteger(total_carpet_area)) {
    total_carpet_area = total_carpet_area;
  } else {
    total_carpet_area = total_carpet_area.toFixed(2);
  }

  $(".total_carpet_area").html(total_carpet_area);
}

function delete_area_working_item(row, itemid) {
  var userConfirmed = confirm("Are you sure you want to delete this item?");
  if (!userConfirmed) {
    return false;
  }
  ...
  // If is edit we need to add to input removed_items to track activity
  if (itemid && $('input[name="isedit"]').length > 0) {
    $("#removed-area-working-items").append(hidden_input("removed_area_working_items[]", itemid));
  }
}

var tabAreaSummaryKeyCounts = 0;
function getAreaSummaryNextItemKey() {
  if (!tabAreaSummaryKeyCounts) {
    tabAreaSummaryKeyCounts = 0;
  }
  tabAreaSummaryKeyCounts += 1;
  return tabAreaSummaryKeyCounts;
}

function add_area_summary_item_to_table(data, itemid, summary_id) {

  var tab = $('.area_summary_tab.active').attr('id');
  var area_id = $('.area_summary_tab.active').attr('data-id');
  data =
    typeof data == "undefined" || data == "undefined"
      ? get_area_summary_item_preview_values(tab)
      : data;

  var table_row = "";
  var item_key = getAreaSummaryNextItemKey();

  var master_area_name = functionality_area_name = "newareasummaryitems[" + item_key + "][master_area]";

  $.when(
    get_master_area_dropdown(master_area_name, data.master_area),
    get_functionality_area_dropdown(functionality_area_name, data.master_area)
  ).done(function (master_area_dropdown, functionality_area_dropdown) {
    table_row += '<tr class="item">';

    table_row +=
      '<input type="hidden" name="newareasummaryitems[' +
      item_key +
      '][area_id]" value=' + area_id + '>';

    if (summary_id == 3) {
      table_row += '<td>' + functionality_area_dropdown[0] + '</td>';
    } else {
      table_row += '<td>' + master_area_dropdown[0] + '</td>';
    }

    table_row +=
      '<td><input type="number" onblur="calculate_area_summary_total();" onchange="calculate_area_summary_total();" name="newareasummaryitems[' +
      item_key +
      '][area]" value="' +
      data.area +
      '" class="form-control" id="area"></td>';

    table_row +=
      '<td><a href="#" class="btn btn-danger pull-left" onclick="delete_area_summary_item(this,' +
      itemid +
      '); return false;"><i class="fa fa-trash"></i></a></td>';
    ...
    setTimeout(function () {
      calculate_area_summary_total();
    }, 15);

    init_selectpicker();
    init_datepicker();
    init_color_pickers();
    clear_area_summary_item_preview_values(tab);
    return true;
  });
  return false;
}

function get_master_area_dropdown(name, value) {
  return $.post(admin_url + "costplanning/get_master_area_dropdown", {
    name: name,
    value: value,
  });
}

function get_area_summary_item_preview_values(tab) {
  var response = {};
  response.master_area = $('#' + tab + ' select[name="master_area"]').selectpicker("val");
  response.area = $('#' + tab + ' input[name="area"]').val();
  return response;
}

function clear_area_summary_item_preview_values(tab) {
  var previewArea = $('#' + tab + '').find("tr").eq(1);
  previewArea.find("textarea").val("");
  previewArea.find('input').val("");
...
function calculate_area_summary_total() {
  var tab = $('.area_summary_tab.active').attr('id');
  var total_area = 0;
  var rows = $('#' + tab + ' tbody.area_summary tr.item');
  $.each(rows, function () {
  ...
  if (Number.isInteger(total_area)) {
...
function delete_area_summary_item(row, itemid) {
  var userConfirmed = confirm("Are you sure you want to delete this item?");
  if (!userConfirmed) {
    return false;
  }
  ...
  // If is edit we need to add to input removed_items to track activity
  if (itemid && $('input[name="isedit"]').length > 0) {
    $("#removed-area-summary-items").append(hidden_input("removed_area_summary_items[]", itemid));
  }
}

function uploaddetailedcostingcsv() {
  "use strict";
  var activeTab = $('.detailed-costing-tab.active');
  var fileInput = activeTab.find('#file_csv')[0];
  ...
  reader.onload = function (e) {
    ...
    if (totalRows > 0) {
      var cleanedData = jsonData.map(row => ({
        description: "",
        long_description: row["Description"]?.trim() || "",
        qty: row["Qty"]?.toString().replace(/,/g, "").trim() || "0",
        rate: row["(*)Rate"]?.toString().replace(/,/g, "").trim() || "0",
      ...
      if (cleanedData.length > 0) {
        insertEstimateRowsSequentially(cleanedData);
        fileInput.value = '';
      }
    }
  };
...
function insertEstimateRowsSequentially(data, delay = 50) {
  let index = 0;
  function next() {
    if (index < data.length) {
      add_estimate_item_to_table(data[index], undefined, undefined, undefined);
      index++;
...
$('a[href="#area_summary"]').on('shown.bs.tab', function (e) {
  calculate_area_summary_total();
});

$('a.tab_sub_area_summary').on('shown.bs.tab', function (e) {
  calculate_area_summary_total();
});

$('a[href="#area_working"]').on('shown.bs.tab', function (e) {
  calculate_area_working_total();
});

$('a.tab_sub_area_working').on('shown.bs.tab', function (e) {
  calculate_area_working_total();
});


// $('body').on('click', '.budgeted-amount-display', function(e) {
//   e.preventDefault();
//   var rowId = $(this).data('id');
//   var invoiceId = $(this).data('invoice');
//   var annexure = $(this).data('annexure');
//   var currentAmount = $(this).text().replace(/[^\d.-]/g, '');
//   $(this).replaceWith('<input type="number" class="form-control budgeted-amount-input" value="' + currentAmount + '" data-invoice="' + invoiceId + '" data-annexure="' + annexure + '" data-id="' + rowId + '" min="0">');
// });

// $('body').on('change', '.budgeted-amount-input', function(e) {
//   e.preventDefault();
...
//   var annexure = $(this).data('annexure');
//   var amount = $(this).val();
//   var budgetedAmount = parseFloat($(this).val()) || 0;
//   var totalPreviousBilling = parseFloat($(`.total-previous-billing-display[data-id="${rowId}"]`).text().replace(/[^\d.-]/g, '')) || 0;
//   var totalCurrentBilling = parseFloat($(`.total-current-billing-amount-display[data-id="${rowId}"]`).text().replace(/[^\d.-]/g, '')) || 0;
//   var totalCumulativeBilling = totalPreviousBilling + totalCurrentBilling;
//   var balanceAvailable = budgetedAmount - totalCumulativeBilling;
//   totalCumulativeBilling = format_money(totalCumulativeBilling);
//   balanceAvailable = format_money(balanceAvailable);

//   $.post(admin_url + 'invoices/update_budgeted_amount_summary', {
//     invoiceid: invoiceId,
//     annexure: annexure,
//     amount: amount
//   }).done(function(response) {
//    response = JSON.parse(response);
//    if (response.success) {
//       $(`.total-cumulative-billing-display[data-id="${rowId}"]`).html('');
//       $(`.total-cumulative-billing-display[data-id="${rowId}"]`).html(totalCumulativeBilling);
//       $(`.balance-available-display[data-id="${rowId}"]`).html('');
//       $(`.balance-available-display[data-id="${rowId}"]`).html(balanceAvailable);
//       alert_float('success', response.message);
//    } else {
//       alert_float('danger', response.message);
//    }
...
$('body').on('click', '.total-previous-billing-display', function (e) {
  e.preventDefault();
  var rowId = $(this).data('id');
  var invoiceId = $(this).data('invoice');
  var annexure = $(this).data('annexure');
  var currentAmount = $(this).text().replace(/[^\d.-]/g, '');
  $(this).replaceWith('<input type="number" class="form-control total-previous-billing-input" value="' + currentAmount + '" data-invoice="' + invoiceId + '" data-annexure="' + annexure + '" data-id="' + rowId + '" min="0">');
});

$('body').on('change', '.total-previous-billing-input', function (e) {
  e.preventDefault();
  ...
  var annexure = $(this).data('annexure');
  var amount = $(this).val();
  var totalPreviousBilling = parseFloat($(this).val()) || 0;
  var budgetedAmount = parseFloat($(`.budgeted-amount-display[data-id="${rowId}"]`).text().replace(/[^\d.-]/g, '')) || 0;
  var totalCurrentBilling = parseFloat($(`.total-current-billing-amount-display[data-id="${rowId}"]`).text().replace(/[^\d.-]/g, '')) || 0;
  var totalCumulativeBilling = totalPreviousBilling + totalCurrentBilling;
  var balanceAvailable = budgetedAmount - totalCumulativeBilling;
  totalCumulativeBilling = format_money(totalCumulativeBilling);
  ...
  $.post(admin_url + 'invoices/update_total_previous_billing_summary', {
    invoiceid: invoiceId,
    annexure: annexure,
    amount: amount
  }).done(function (response) {
...
$('body').on('dblclick', '.tab_sub_area_working', function (e) {
  e.preventDefault();
  var $this = $(this);
  var currentTitle = $this.text().trim();
  var area_id = $this.data('tab-id');
  if ($this.find('input').length) return;
  var $input = $('<input type="text" class="form-control" />').val(currentTitle);
  $this.html($input);
  $input.focus();
  $input.on('blur keypress', function (e) {
    if (e.type === 'blur' || e.which === 13) {
      ...
      $.post(admin_url + 'estimates/update_area_statement_tabs', { id: area_id, name: newTitle }, function (response) {
...
$('body').on('click', '#add_new_area_working_tab', function (e) {
  e.preventDefault();
  var $this = $(this);
  var estimate_id = $this.data('estimate-id');
  $('#add_new_area_working_tab').find('input').remove();
  var $input = $('<input type="text" class="form-control" placeholder="Enter tab title..." style="width: 200px; display:inline;" />');
  $('#add_new_area_working_tab a').html($input);
  $input.focus();
  $input.on('blur keypress', function (e) {
    if (e.type === 'blur' || e.which === 13) {
      var newTitle = $input.val().trim();
      if (newTitle === '') {
        ...
        $('#add_new_area_working_tab a').html('<strong>➕</strong>');
        return;
      }
      $.post(admin_url + 'estimates/add_area_statement_tabs', { estimate_id: estimate_id, name: newTitle }, function (response) {
        if (response.group_id) {
          location.reload();
        } else {
...
$('body').on('click', '.delete-area-working-tab', function (e) {
  e.preventDefault();
  var $tab = $(this).closest('a');
  var tabId = $tab.data('tab-id');
  var $li = $tab.closest('li');
  if (confirm('Are you sure you want to delete this tab?')) {
    $.post(admin_url + 'estimates/delete_area_statement_tabs', { id: tabId }, function (response) {
      if (response.group_id) {
        location.reload();
      }
    }, 'json');
...
displayAreaSummaryUnit();

$('body').on('change', 'input[name="show_as_unit"]', function (e) {
  displayAreaSummaryUnit();
});

function displayAreaSummaryUnit() {
  var selectedLabel = $('input[name="show_as_unit"]:checked').next('label').text();
  $('.show_as_unit_name').text(selectedLabel);
}

displayAreaWorkingUnit();

$('body').on('change', 'input[name="show_aw_unit"]', function (e) {
  displayAreaWorkingUnit();
});

function displayAreaWorkingUnit() {
  var selectedLabel = $('input[name="show_aw_unit"]:checked').next('label').text();
  var selectedLabelupdated = selectedLabel.replace('2', '');
  $('.show_aw_unit_name').text(selectedLabel);
  $('.show_aw_unit_se').text(selectedLabelupdated);
}

function get_functionality_area_dropdown(name, value) {
  return $.post(admin_url + "costplanning/get_functionality_area_dropdown", {
    name: name,
    value: value,
  });
}

function uploadareaworkingcsv() {
  "use strict";
  var fileInput = $('#area_working_file_csv')[0];
  var file = fileInput?.files[0];
  if (!file) {
    alert("Please select a file.");
    return;
  ...
  reader.onload = function (e) {
    ...
    if (totalRows > 0) {
      var cleanedData = jsonData.map(row => ({
        area_description: row["Room/Spaces"]?.trim() || "",
        area_length: row["Length"]?.toString() || "0",
        area_width: row["Width"]?.toString() || "0",
      }));
      if (cleanedData.length > 0) {
        insertAreaWorkingRowsSequentially(cleanedData);
        fileInput.value = '';
      }
    }
  };
...
function insertAreaWorkingRowsSequentially(data, delay = 50) {
  let index = 0;
  function next() {
    if (index < data.length) {
      add_area_working_item_to_table(data[index], undefined, undefined, undefined);
      index++;
      setTimeout(next, delay);
    }
  }
...
$("body").on("change", "#overall_budget_area_id", function (e) {
  calculate_estimate_total();
});

function create_new_revision(id) {
  var userConfirmed = confirm("Are you sure you want to revise this estimate?");
  if (!userConfirmed) {
    return false;
  }
  $.post(admin_url + "estimates/create_new_revision", {
    id: id,
  }).done(function (response) {
    response = JSON.parse(response);
    if (response && response.new_revision_id) {
      alert_float("success", "Revise this estimate have Successfully");
      window.location.href = admin_url + "estimates";
    } else {
      alert_float("warning", "Failed to create a new revision.");
    }
  }).fail(function () {
    alert_float("warning", "An error occurred while creating the revision.");
  });
}

// Table preports
function init_table_preports(manual) {
  // Single form is for other preports from user
  if (
    typeof manual == "undefined" &&
    ($("body").hasClass("dashboard") || $("body").hasClass("single-form"))
  ) {
  ...
  if ($("body").find(".preports-table").length === 0) {
    return;
  }
  var PreportserverParams = {},
    preports_Filters = $("._hidden_inputs._filters.preports_filters input");
  var preports_date_created_index = $(
    "table.preports-table thead .report_created_column"
  ).index();
  $.each(preports_Filters, function () {
    PreportserverParams[$(this).attr("name")] =
      '[name="' + $(this).attr("name") + '"]';
  });
  PreportserverParams["project_id"] = '[name="project_id"]';
  var preportsTableNotSortable = [0]; // bulk actions
  var pathSegments = window.location.pathname.split('/');
  var lastSegment = pathSegments[pathSegments.length - 1] || 'dpr';
  var _preports_table_url = admin_url + "forms/progress_report_listing/" + lastSegment;
  if ($("body").hasClass("preports-page")) {
    _preports_table_url += "?bulk_actions=true";
  }
  _table_api = initDataTable(
    ".preports-table",
    _preports_table_url,
    preportsTableNotSortable,
    preportsTableNotSortable,
    PreportserverParams,
    [preports_date_created_index, "desc"]
  );
  if (_table_api && $("body").hasClass("dashboard")) {
    var notVisibleDashboardDefault = [4, preports_date_created_index, 5, 6];
    for (var i in notVisibleDashboardDefault) {
      _table_api.column(notVisibleDashboardDefault[i]).visible(false, false);
    }
    _table_api.columns.adjust();
...
function change_default_project(projectId) {
  Swal.fire({
    title: 'Are you sure?',
    text: "Do you want to change the project?",
    icon: 'warning',
    showCancelButton: true,
    ...
    if (result.isConfirmed) {
      $.post(admin_url + "dashboard/change_default_project", {
        id: projectId,
      }).done(function (response) {
        Swal.fire(
          'Changed!',
          'Default project has been changed.',
          'success'
        ).then(() => {
          let currentUrl = window.location.href;
          if (currentUrl.includes("drawing_management")) {
            // Reset URL to base drawing_management
            window.location.href = currentUrl.split("drawing_management")[0] + "drawing_management";
          } else {
            location.reload();
          }
        });
        ...
        Swal.fire(
          'Error!',
          'An error occurred while changing the project.',
          'error'
        );
      });
    }
...
var default_project = $('#default_project').val();
if (default_project != null && default_project != '') {
  var $projectSelects = $('select[name="project"], select[name="project_id"], select[name="projects"], select[name="project_filter"], select[name="projects[]"], select[name="project[]"], select[name="select_project"]');
  $projectSelects.each(function () {
    var $this = $(this);
    $this.val(default_project).trigger('change');
    $this.selectpicker('refresh');
    $this.parent('.bootstrap-select').css({
      'pointer-events': 'none',
      'background-color': '#e9ecef'
    });
    $this.parent('.bootstrap-select').find('button').attr('tabindex', '-1');
  });
}
...
// Get all payment certificate notes
function get_pc_notes(id, rel_type) {
  requestGet("purchase/get_pc_notes/" + id + "/" + rel_type).done(function (response) {
    $("#pc_notes_area").html(response);
    var totalNotesNow = $("#pc-notes-wrapper").attr("data-total");
    if (totalNotesNow > 0) {
      $(".notes-total")
        .html('<span class="badge">' + totalNotesNow + "</span>")
        .removeClass("hide");
...
// Delete payment certificate note
function delete_pc_note(wrapper, id) {
  if (confirm_delete()) {
    requestGetJSON("purchase/delete_pc_note/" + id).done(function (response) {
      if (response.success === true || response.success == "true") {
        location.reload();
      }
    });
...
/* Global function for editing payment certificate note */
function toggle_edit_pc_note(id) {
  $("body")
    .find('[data-note-edit-textarea="' + id + '"]')
    .toggleClass("hide");
  $("body")
...
// Global function to edit payment certificate note
function edit_pc_note(id) {
  var description = $("body")
    .find('[data-note-edit-textarea="' + id + '"] textarea')
    .val();
  if (description !== "") {
    $.post(admin_url + "purchase/edit_pc_note/" + id, {
      description: description,
    }).done(function (response) {
      response = JSON.parse(response);
      if (response.success === true || response.success == "true") {
