<script>
  ...
  $('.account-template-form-submiter').on('click', function() {
    ...
    function edit_approval_setting(invoker,id){
      ...
      $.post(admin_url+'purchase/find_project_members').done(function(response){
        response = JSON.parse(response);
...
function allow_vendors_to_register(invoker){
  ...
  function uploadmasterareafilecsv() {
    "use strict";
    if (($("#file_csv").val() != '') && ($("#file_csv").val().split('.').pop() == 'xlsx')) {
     ...
     $.ajax({
        url: admin_url + 'estimates/import_file_xlsx_master_area',
        method: 'post',
        data: formData,
        contentType: false,
        processData: false
  ...
  function uploadfunctionalityareafilecsv() {
    "use strict";
    if (($("#file_csv").val() != '') && ($("#file_csv").val().split('.').pop() == 'xlsx')) {
     var formData = new FormData();
     formData.append("file_csv", $('#file_csv')[0].files[0]);
     ...
     $.ajax({
        url: admin_url + 'estimates/import_file_xlsx_functionality_area',
        method: 'post',
        data: formData,
        contentType: false,
        processData: false
