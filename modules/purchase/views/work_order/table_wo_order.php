...
if(isset($purchase_dashboard)){
    array_push($where, 'AND order_date >= "'.date('Y-m-d', strtotime('-1 month')).'"');
    array_push($where, 'AND order_date <= "'.date('Y-m-d').'"');
}
...
$result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, [db_prefix().'wo_orders.id as id','company','wo_order_number','expense_convert',db_prefix().'projects.name as project_name',db_prefix().'departments.name as department_name', db_prefix().'currencies.name as currency_name', 'currency', '(SELECT GROUP_CONCAT(' . db_prefix() . 'project_members.staff_id SEPARATOR ",") FROM ' . db_prefix() . 'project_members WHERE ' . db_prefix() . 'project_members.project_id=' . db_prefix() . 'wo_orders.project) as member_list'], '', [], $having);
