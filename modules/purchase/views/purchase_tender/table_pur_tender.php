...
$aColumns = [
    ...
    'status',
    // 'project',
    db_prefix() . 'pur_tender' . '.id',
];
$sIndexColumn = 'id';
$sTable       = db_prefix() . 'pur_tender';
$join         = [
    'LEFT JOIN ' . db_prefix() . 'departments ON ' . db_prefix() . 'departments.departmentid = ' . db_prefix() . 'pur_tender.department',
    'LEFT JOIN ' . db_prefix() . 'items_groups ON ' . db_prefix() . 'items_groups.id = ' . db_prefix() . 'pur_tender.group_pur',
    'LEFT JOIN ' . db_prefix() . 'wh_sub_group ON ' . db_prefix() . 'wh_sub_group.id = ' . db_prefix() . 'pur_tender.sub_groups_pur',
    // 'LEFT JOIN '.db_prefix().'area ON '.db_prefix().'area.id = '.db_prefix().'pur_tender.area_pur',
];
$where = [];
...
if (isset($project)) {
    array_push($where, ' AND ' . db_prefix() . 'pur_tender.project = ' . $project);
}
...
if (!has_permission('purchase_request', '', 'view')) {
    ...
    array_push($where, 'AND (' . db_prefix() . 'pur_tender.requester = ' . get_staff_user_id() .  $or_where . ' OR ' . get_staff_user_id() . ' IN (SELECT staffid FROM ' . db_prefix() . 'pur_approval_details WHERE ' . db_prefix() . 'pur_approval_details.rel_type = "pur_tender" AND ' . db_prefix() . 'pur_approval_details.rel_id = ' . db_prefix() . 'pur_tender.id))');
}
...
foreach ($rResult as $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        if ($aColumns[$i] == 'request_date') {
            ...
            $name .= '<a href="' . admin_url('purchase/view_pur_tender/' . $aRow['id']) . '" >' . _l('view') . '</a>';

            if ((has_permission('purchase_tender', '', 'edit') || is_admin()) &&  $aRow['status'] != 2) {
            ...
            $_data = $name;
        } elseif ($aColumns[$i] == db_prefix() . 'pur_tender' . '.id') {
            if ($aRow['status'] == 2) {
        ...
        } elseif ($aColumns[$i] == 'pur_tn_code') {
            $_data = '<a href="' . admin_url('purchase/view_pur_tender/' . $aRow['id']) . '">' . $aRow['pur_tn_code'] . '</a>';
        } elseif ($aColumns[$i] == 'project') {

            // $_data = get_po_html_by_pur_tender($aRow['id']);
