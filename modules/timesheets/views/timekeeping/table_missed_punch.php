...
$aColumns = [
    ...
    'additional_day',
    '(SELECT GROUP_CONCAT(staffid SEPARATOR ",") FROM '.db_prefix().'timesheets_approval_details WHERE rel_id = '.db_prefix().'timesheets_missed_punch.id and rel_type = "additional_timesheets") as approver',
    'status',
];
$sIndexColumn = 'id';
$sTable       = db_prefix().'timesheets_missed_punch';
$join = ['LEFT JOIN '.db_prefix().'staff b ON b.staffid = '.db_prefix().'timesheets_missed_punch.creator',];
$where = [];
...
if(!is_admin() && !has_permission('additional_timesheets_management','','view')){
    if(!$this->ci->input->post('chose_ats') || $this->ci->input->post('chose_ats') == 'all'){
        array_push($where, timesheet_staff_manager_query('additional_timesheets_management', 'creator', 'AND'));
           array_push($where,'OR ('. get_staff_user_id() .' in (select staffid from '.db_prefix().'timesheets_approval_details where rel_type = "additional_timesheets" and rel_id = '.db_prefix().'timesheets_missed_punch.id))');        
   }

   array_push($where,'OR ('. db_prefix() .'timesheets_missed_punch.staff_id =  '.get_staff_user_id().')');        

}

if($this->ci->input->post('status_filter_mp')){
    $where_status = '';
    $status = $this->ci->input->post('status_filter_mp');
...
if($this->ci->input->post('department_mp')){
    $where_dpm = '';
    $department = $this->ci->input->post('department_mp');
...
if($this->ci->input->post('rel_type_filter_mp')){
    $where_rel_type = '';
    $rel_type = $this->ci->input->post('rel_type_filter_mp');
...
if($this->ci->input->post('chose_mp')){
    $chose = $this->ci->input->post('chose_mp');
    $sql_where = '';
    if($chose != 'all'){
        if($sql_where != ''){
            $sql_where .= ' AND ("'.get_staff_user_id().'" IN (SELECT staffid FROM '.db_prefix().'timesheets_approval_details where '.db_prefix().'timesheets_approval_details.rel_type IN ("additional_timesheets") AND '.db_prefix().'timesheets_approval_details.rel_id = '.db_prefix().'timesheets_missed_punch.id ))';
        }else{
            $sql_where .= '("'.get_staff_user_id().'" IN (SELECT staffid FROM '.db_prefix().'timesheets_approval_details where '.db_prefix().'timesheets_approval_details.rel_type IN ("additional_timesheets") AND '.db_prefix().'timesheets_approval_details.rel_id = '.db_prefix().'timesheets_missed_punch.id ))';
        }
    }else{
        $sql_where = '';
    }
...
foreach ($rResult as $aRow) {
    ...
    $row[] = date('d M, Y', strtotime($aRow['additional_day']));
    ...
    $membersOutput .= '<span class="hide">' . ts_trim($exportMembers, ', ') . '</span>';
  ...
  $options = '<a href="Javascript:void(0);" onclick="view_missed_punch('.$aRow['id'].'); return false" class="btn btn-default btn-icon" data-toggle="sidebar-right" data-target=".additional-timesheets-sidebar"><i class="fa fa-eye"></i></a>';
