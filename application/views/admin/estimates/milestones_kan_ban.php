...
foreach ($milestones as $milestone) {
    ...
    $total_pages = ceil($this->estimates_model->do_milestones_kanban_query($milestone['id'], $estimate_id, 1, $milestonesTasksWhere, true) / get_option('tasks_kanban_limit'));

    $tasks       = $this->estimates_model->do_milestones_kanban_query($milestone['id'], $estimate_id, 1, $milestonesTasksWhere);
...
<ul class="kan-ban-col milestone-column<?php if (staff_cant('edit_milestones', 'projects') || $milestone['id'] == 0) {
   ...
   <a href='#' class='tw-block reset_milestone_color <?php if ($milestone_color == '') {
    ...
    foreach ($tasks as $task) {
        $this->load->view('admin/estimates/_milestone_kanban_card', ['task' => $task, 'milestone' => $milestone['id']]);
    } ?>
