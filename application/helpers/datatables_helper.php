...
function data_tables_purchase_tracker_init($aColumns, $join = [], $where = [], $additionalSelect = [], $sGroupBy = '', $having = '')
{
    ...
    SELECT *, 
        CASE 
            WHEN type = 1 THEN 
                CASE 
                    WHEN ( SELECT COALESCE(SUM(po_quantities), 0) FROM tblgoods_receipt_detail WHERE goods_receipt_id = combined_orders.id ) = ( SELECT COALESCE(SUM(quantities), 0) FROM tblgoods_receipt_detail WHERE goods_receipt_id = combined_orders.id ) THEN '2' 
                    WHEN ( SELECT COALESCE(SUM(quantities), 0) FROM tblgoods_receipt_detail WHERE goods_receipt_id = combined_orders.id ) = 0 THEN '0' 
                    WHEN ( SELECT COALESCE(SUM(quantities), 0) FROM tblgoods_receipt_detail WHERE goods_receipt_id = combined_orders.id ) > 0 THEN '1' 
                    ELSE '0' 
                END 
            ELSE '0' 
    ...
    FROM ( 
        SELECT 
            id, 
            goods_receipt_code, 
            supplier_code AS supplier_name, 
            buyer_id, 
            kind, 
            pr_order_id,
            wo_order_id, 
...
function data_tables_init_for_billing_summary_reports($aColumns, $sIndexColumn, $combinedTables, $join = [], $where = [], $additionalSelect = [], $sGroupBy = '', $searchAs = [], $having = '', $module = '')
{
    $CI = &get_instance();
    $data = $CI->input->post();
    ...
    $sTable = "(
        SELECT 
            ...
            COALESCE(SUM(pi.final_certified_amount), 0) AS total_billed,
            COALESCE(SUM(it.qty * it.rate), 0) AS total_paid,
            (COALESCE(SUM(pi.final_certified_amount), 0) - COALESCE(SUM(it.qty * it.rate), 0)) AS total_balance,
            CASE 
                WHEN COALESCE(SUM(pi.final_certified_amount), 0) > 0 THEN 
                    ROUND((COALESCE(SUM(it.qty * it.rate), 0) * 100) / COALESCE(SUM(pi.final_certified_amount), 0), 2)
                ELSE 0 
...
function data_tables_init_for_billing_invoicing_reports($aColumns, $sIndexColumn, $combinedTables, $join = [], $where = [], $additionalSelect = [], $sGroupBy = '', $searchAs = [], $having = '', $module = '')
{
    $CI = &get_instance();
    $data = $CI->input->post();
    ...
    $sTable = "(
        SELECT 
            ...
            MAX(pi.invoice_date) AS last_bill_date,
            CASE 
                WHEN (COALESCE(SUM(pi.final_certified_amount), 0) - COALESCE(SUM(it.qty * it.rate), 0)) = 0 THEN 'Paid'
...
function data_tables_budget_usage_init($aColumns, $join = [], $where = [], $additionalSelect = [], $sGroupBy = '', $having = '')
{
    $CI = &get_instance();
    $data = $CI->input->post();
    ...
    $sTable = "(
        SELECT combined_orders.*
        FROM (
            SELECT 
                po.id AS id,
                po.pur_order_number AS order_number,
                po.pur_order_name AS description,
                po.group_pur AS budget_head,
                ig.name AS budget_head_name,
                po.vendor AS vendor_id,
                pv.company AS vendor_name,
                po.total AS amount,
                po.estimate AS estimate,
                'po' AS type
            FROM tblpur_orders po
            LEFT JOIN tblitems_groups ig ON ig.id = po.group_pur
            LEFT JOIN tblpur_vendor pv ON pv.userid = po.vendor
            ...
            SELECT 
                wo.id AS id,
                wo.wo_order_number AS order_number,
                wo.wo_order_name AS description,
                wo.group_pur AS budget_head,
                ig.name AS budget_head_name,
                wo.vendor AS vendor_id,
                pv.company AS vendor_name,
                wo.total AS amount,
                wo.estimate AS estimate,
                'wo' AS type
            FROM tblwo_orders wo
            LEFT JOIN tblitems_groups ig ON ig.id = wo.group_pur
            LEFT JOIN tblpur_vendor pv ON pv.userid = wo.vendor
            ...
            SELECT 
                co.id AS id,
                co.pur_order_number AS order_number,
                co.pur_order_name AS description,
                co.group_pur AS budget_head,
                ig.name AS budget_head_name,
                co.vendor AS vendor_id,
                pv.company AS vendor_name,
                co.total AS amount,
                co.estimate AS estimate,
                'co' AS type
            FROM tblco_orders co
            LEFT JOIN tblitems_groups ig ON ig.id = co.group_pur
            LEFT JOIN tblpur_vendor pv ON pv.userid = co.vendor
        ) AS combined_orders
    ) AS final_result";
