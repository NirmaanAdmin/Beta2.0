    ...
    public function pdf($id)
{
    ...
    try {
            ...
            foreach ($dms_items as $key => $dms_id) {
                try {
                    ...
                    if ($item) {
                        ...
                        if (strtolower($extension) === 'pdf') {
                            $file_path = FCPATH . str_replace(base_url(), '', 
                                DRAWING_MANAGEMENT_PATH . 'files/' . $item->parent_id . '/' . $item->name);
        ...
        // Use Ghostscript for merging if available
        if (!empty($extraFiles) && $this->hasGhostscript()) {
            $mergedFile = $this->mergePdfsWithGhostscript($pdf, $extraFiles);
            
            if ($mergedFile) {
                // Output the merged file
                header('Content-Type: application/pdf');
                header('Content-Disposition: attachment; filename="' . mb_strtoupper(slug_it($ticket->subject)) . '.pdf"');
                readfile($mergedFile);
                @unlink($mergedFile);
                return;
            }
        ...
        // Fallback: Try FPDI with error handling
        if (!empty($extraFiles)) {
            $merged = $this->mergePdfsWithFpdi($pdf, $extraFiles);
            if ($merged) {
                $pdf = $merged;
            }
        }
...
private function mergePdfsWithGhostscript($ticketPdf, $extraFiles)
{
    // Save ticket PDF to temp file
    $ticketPdfString = $ticketPdf->Output('', 'S');
    $tempDir = sys_get_temp_dir();
    $tempTicket = tempnam($tempDir, 'ticket_') . '.pdf';
    file_put_contents($tempTicket, $ticketPdfString);
    
    // Prepare all files for merging
    $allFiles = array_merge([$tempTicket], $extraFiles);
    $mergedFile = tempnam($tempDir, 'merged_') . '.pdf';
    
    // Build Ghostscript command
    $cmd = 'gs -q -dNOPAUSE -dBATCH -sDEVICE=pdfwrite -sOutputFile="' . $mergedFile . '" ';
    ...
    // Clean up temp ticket file
    @unlink($tempTicket);
    
    if ($return === 0 && file_exists($mergedFile) && filesize($mergedFile) > 0) {
        return $mergedFile;
    }
...
private function mergePdfsWithFpdi($ticketPdf, $extraFiles)
{
    try {
        $fpdiBase = APPPATH . 'third_party/fpdi/';
        if (file_exists($fpdiBase . 'src/autoload.php')) {
        ...
        $ticketPdfString = $ticketPdf->Output('', 'S');
        $mergedPdf = new \setasign\Fpdi\Tcpdf\Fpdi('P', 'mm', 'A4', true, 'UTF-8', false);
        $mergedPdf->setPrintHeader(false);
        $mergedPdf->setPrintFooter(false);
        
        // Add ticket PDF
        $src = \setasign\Fpdi\PdfParser\StreamReader::createByString($ticketPdfString);
        $pageCount = $mergedPdf->setSourceFile($src);
        for ($p = 1; $p <= $pageCount; $p++) {
            $tplId = $mergedPdf->importPage($p);
            $size  = $mergedPdf->getTemplateSize($tplId);
            $orientation = ($size['width'] > $size['height']) ? 'L' : 'P';
            $mergedPdf->AddPage($orientation, [$size['width'], $size['height']]);
            $mergedPdf->useTemplate($tplId, 0, 0, $size['width'], $size['height'], true);
        }
        
        // Try to add each extra file, skip if fails
        foreach ($extraFiles as $file) {
            ...
            try {
                ...
                continue; // Skip incompatible PDFs
            }
        }
        ...
        log_message('error', 'FPDI merging failed: ' . $e->getMessage());
        return false;
    }
}
