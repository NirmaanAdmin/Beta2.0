...
class Invoices_model extends App_Model
{
    ...
    public function add($data, $expense = false)
    {
        ...
        $data['suppliers_gst'] = get_default_company_gst_id();
    ...
    public function update($data, $id)
    {
        ...
        if (isset($data['cgst_type'])) {
            if ($data['cgst_type'] == 1) {
                $data['cgst_amount'] = 0;
            } elseif ($data['cgst_type'] == 2) {
                $data['cgst_percentage'] = 0;
            }
        }
        if (isset($data['sgst_type'])) {
            if ($data['sgst_type'] == 1) {
                $data['sgst_amount'] = 0;
            } elseif ($data['sgst_type'] == 2) {
    ...
    public function get_annexure_invoice_details($invoiceid, $ignore_management_fess = false, $previous_billing_summary = true)
    {
        ...
        foreach ($indexa as $key => $value) {
            ...
            if($invoice->cgst_type == 1) {
                $final_invoice['cgst_tax'] = $final_invoice['subtotal'] * ($invoice->cgst_percentage / 100);
            } else if($invoice->cgst_type == 2) {
                $final_invoice['cgst_tax'] = $invoice->cgst_amount;
            } else {
                $final_invoice['cgst_tax'] = 0;
            }
            if($invoice->sgst_type == 1) {
                $final_invoice['sgst_tax'] = $final_invoice['subtotal'] * ($invoice->sgst_percentage / 100);
            } else if($invoice->sgst_type == 2) {
                $final_invoice['sgst_tax'] = $invoice->sgst_amount;
            } else {
                $final_invoice['sgst_tax'] = 0;
            }
            $final_invoice['tax'] = $final_invoice['cgst_tax'] + $final_invoice['sgst_tax'];
            $final_invoice['amount'] = $final_invoice['subtotal'] + $final_invoice['tax'];
            $final_invoice['remarks'] = $invoice->remarks;
            if ($value['annexure'] != 17) {
                $final_invoice['total_without_man_fees'] += $value['subtotal'];
            }
        }
        ...
        if($previous_billing_summary) {
            ...
            foreach ($all_annexures as $akey => $avalue) {
                ...
                if(!empty($invoice_budget_summary)) {
                    $total_previous_billing = $invoice_budget_summary['total_previous_billing'];
                }
        ...
        $total_budget_summary['total_without_man_fees'] = 0;
        if(!empty($budgetsummary)) {
            foreach ($budgetsummary as $key => $value) {
                ...
                if ($value['annexure'] != 17) {
                    $total_budget_summary['total_without_man_fees'] += $value['total_current_billing_amount'];
                }
            }
        }
    ...
    public function get_previous_billing_amount($invoice)
    {
        ...
        if (!empty($result)) {
            foreach ($result as $key => $value) {
                $single_invoice = $this->get_annexure_invoice_details($value['id'], false, false);
    ...
    public function update_invoice_budget_summary($invoiceid, $annexure, $total_previous_billing)
    {
        $response = array();
        $this->db->where('invoiceid', $invoiceid);
        $this->db->where('annexure', $annexure);
        $result = $this->db->get(db_prefix() . 'invoice_budget_summary')->row();
        if(!empty($result)) {
            $response['total_previous_billing'] = $result->total_previous_billing;
            return $response;
        } else {
            $input = array();
            $input['invoiceid'] = $invoiceid;
            $input['annexure'] = $annexure;
            $input['total_previous_billing'] = $total_previous_billing;
            $this->db->insert(db_prefix() . 'invoice_budget_summary', $input);
            $response['total_previous_billing'] = $total_previous_billing;
            return $response;
        }
    }
    ...
    public function get_invoice_export_data($invoiceid)
    {
        ...
        $tblfinvoicehtml .= '
          <tr>
            ...
            <th>'._l('cgst_tax').$cgst_tax_format.'</th>
            <th>'._l('sgst_tax').$sgst_tax_format.'</th>
